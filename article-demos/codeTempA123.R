...
#+ nu-plot, fig.width=11, fig.height=6, message=FALSE, warning=FALSE
 ...
 #' NEW 3. Create categories of temperature: 3 states
 #'
 #'
 #'
 data("tempdata")
 ...
 x1 <- min(temp$MAXTEMP)
 maxAmp <- xn - x1
 temp$MAXTEMP123  <- ifelse(temp$MAXTEMP < x1 + maxAmp/3, 1,
                            ifelse(temp$MAXTEMP < x1 + 2*maxAmp/3, 2, 3)
                            )


 head(temp, 4)
 prop.table(table(temp$MAXTEMP123)) # frequency of thermal regimes
 #'
 #' 4. Run FS for temp:
 #'
 Temp123 <- rev(temp$MAXTEMP123)
 #' hdMTD functions assume the sample is sorted from the latest observations to
 #' oldest.
 #'
 recompute <- FALSE
 ...
 if (recompute) {
   message("Recomputing...")
   FS_Temp123_d400 <- hdMTD_FS(Temp123, d = 400, l = 3) #takes ~7min.
   #precomputed$FS_Temp123_d400 <- FS_Temp123_d400
 } else {
   message("Using pre-computed data.")
   FS_Temp123_d400 <- precomputed$FS_Temp123_d400
 }
 print(FS_Temp123_d400)
 #' 1,11,353
 #'
 #' Note: The next code line is mentioned in the article but without a CodeChunk <br>
 #' Reduce maximum order to improve estimation <br>
 ...
 if (recompute) {
   message("Recomputing...")
   FS_Temp123_d353 <- hdMTD_FS(Temp123, d = 353, l = 3) #takes ~6min.
   #precomputed$FS_Temp123_d364 <- FS_Temp123_d364
 } else {
   message("Using pre-computed data.")
   FS_Temp123_d353 <- precomputed$FS_Temp123_d353
 }
 print(FS_Temp123_d353)
 # 1,11,353
 #'
 #' 5. Split sample in Train and Test data:
 #'
 ndays <- nrow(temp %>%
                 filter(DATE >= "2023-09-01")) # 366 days in the latest year of the sample
 Temp123_Train <- Temp123[-seq_len(ndays)] # Training data with 4775 days
 Temp123_Test <- Temp123_Train[seq_len(ndays)] # Test data
 #'
 #' 6. Rerun FS for Train data:
 #'
 recompute <- FALSE
 ...
 if (recompute) {
   message("Recomputing...")
   FS_Temp123Train <- hdMTD_FS(Temp123_Train, d = 353, l = 3) #takes ~5min.
   #precomputed$FS_Temp123Train <- FS_Temp123Train
 } else {
   message("Using pre-computed data.")
   FS_Temp123Train <- precomputed$FS_Temp123Train
 }
 print(FS_Temp123Train)
 # 1,353,9

 #' 7. Trim out irrelevant lags:
 #'
 #' With CUT method
 hdMTD_CUT(Temp123_Train, d = 353, S = c(1, 353, 9))
 #' With BIC method
 hdMTD_BIC(Temp123_Train, d = 353, S = c(1, 353, 9), minl = 1, maxl = 3,
           byl = TRUE, BICvalue = TRUE )
 #'
 #' 8. Lag selection with FSC method:
 #'
 ...
 if (recompute) {
   message("Recomputing...")
   FSC_Temp123Train <- hdMTD_FSC(Temp123_Train, d = 353, l = 3) #takes ~3min.
   #precomputed$FSC_Temp123Train <- FSC_Temp123Train
 } else {
   message("Using pre-computed data.")
   FSC_Temp123Train <- precomputed$FSC_Temp123Train
 }
 print(FSC_Temp123Train)
 # 1, 12, 352
 #'
 #' 9. Estimated transition matrix for FS method output:
 #'
 P_FS <- empirical_probs(Temp123_Train, S = c(1, 9, 353), matrixform = T)
 P_FS
 #'


 ct <- countsTab(Temp123_Train, d = 6) # Table with size 6 sequence counts
 head(ct,4)
 #'
 #' MC1 (does not explicitly appear in the article)
 ft <- freqTab(S = 1, A = c(1, 2, 3), countsTab = ct)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^1 * 2
 BICMC1 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC1
 #' Comparable BIC if the model is a Markov chain of order $1$: $1869.162$
 #'
 #' MC2 (does not explicitly appear in the article)
 ft <- freqTab(S = c(1, 2), A = c(1, 2, 3), countsTab = ct)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^2 * 2
 BICMC2 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC2
 #' Comparable BIC if the model is a Markov chain of order $2$: $1850.598$
 #'
 #' MC3. Used as example in the article
 ft <- freqTab(S = c(1, 2, 3), A = c(1, 2, 3), countsTab = ct)
 head(ft, 4)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^3 * 2
 BICMC3 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC3
 #' Comparable BIC if the model is a Markov chain of order $3$: $1854.029$
 #'
 #' MC4 (does not explicitly appear in the article)
 ft <- freqTab(S = c(1, 2, 3, 4), A = c(1, 2, 3), countsTab = ct)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^4 * 2
 BICMC4 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC4
 #' Comparable BIC if the model is a Markov chain of order $4$: $1877.888$
 #'
 #' MC5 (does not explicitly appear in the article)
 ft <- freqTab(S = c(1, 2, 3, 4, 5), A = c(1, 2, 3), countsTab = ct)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^5 * 2
 BICMC5 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC5
 #' Comparable BIC if the model is a Markov chain of order $5$: $1925.962$
 #'
 #' MC6 (does not explicitly appear in the article)
 ft <- freqTab(S = c(1, 2, 3, 4, 5, 6), A = c(1, 2, 3), countsTab = ct)
 pos <- which(ft$Nxa_Sj > 0)
 LL <- sum(log(ft$qax_Sj[pos]) * ft$Nxa_Sj[pos])
 freeParam <- 3^6 * 2
 BICMC6 <- -LL + 0.5 * log(length(Temp123_Train)) * freeParam
 BICMC6
 #' Comparable BIC if the model is a Markov chain of order $6$: $2031.679$
 #'
 #' 11. Comparing models:
 ...
 #'
 #' Estimated matrix for $S=\{-2,-1\}$
 P_MC1 <- empirical_probs(Temp123_Train, S = c(1), matrixform = TRUE)
 P_MC1
 #' Independent model distribution:
 P_Ind <- prop.table(table(Temp123_Train))
 P_Ind
 #'
 #'
 Days1 <- which(Temp123_Test == 1)
 lenDays1 <- length(Days1)
 lenDays1 / ndays # frequency of low temperature days
 Temp123_Test <- c(Temp123_Test, Temp123_Train[seq_len(353)])
 #'
 set.seed(1)
 hitInd <- numeric(1000)
 hitMC1 <- numeric(1000)
 ...
 F1MC1 <- numeric(1000)
 F1FS <- numeric(1000)
 pasts1 <- rownames(P_MC1)
 pastsFS <- rownames(P_FS)
 #'
 for (j in seq_len(1000)){
   u <- runif(ndays)
   predInd <- numeric(ndays)
   predMC1 <- numeric(ndays)
   predFS <- numeric(ndays)

   for (i in ndays:1) {
     predInd[i] <- ifelse(u[i] <= P_Ind[1], 1,
                          ifelse(u[i] <= P_Ind[2], 2, 3)
                          )
     pastRow <- which(pasts1 == paste0(rev(Temp123_Test[i+1]), collapse = ""))
     predMC1[i] <- ifelse(u[i] <= P_MC1[pastRow, 1], 1,
                          ifelse(u[i] <= P_MC1[pastRow, 2], 2, 3)
                          )
     pastRow <- which(pastsFS == paste0(rev(Temp123_Test[c(i+1, i+9, i+353)]), collapse = ""))
     predFS[i] <- ifelse(u[i] <= P_FS[pastRow, 1], 1,
                         ifelse(u[i] <= P_FS[pastRow, 2], 2, 3)
                         )
   }

   hitInd[j] <- sum(predInd == Temp123_Test[seq_len(ndays)])
   hitMC1[j] <- sum(predMC1 == Temp123_Test[seq_len(ndays)])
   hitFS[j] <- sum(predFS == Temp123_Test[seq_len(ndays)])
   T1Ind[j] <- sum(predInd[Days1] == 1)
   T1MC1[j] <- sum(predMC1[Days1] == 1)
   T1FS[j] <- sum(predFS[Days1] == 1)
   F1Ind[j] <- sum(predInd[-Days1] == 1)
   F1MC1[j] <- sum(predMC1[-Days1] == 1)
   F1FS[j] <- sum(predFS[-Days1] == 1)
 }

 #' ### Accuracy
 AccInd <- mean(hitInd)/ndays
 AccMC1 <- mean(hitMC1)/ndays
 AccFS <- mean(hitFS)/ndays
 AccInd; AccMC1; AccFS

 #' ### Precision
 PrecInd <- mean(T1Ind/(T1Ind + F1Ind))
 PrecMC1 <- mean(T1MC1/(T1MC1 + F1MC1))
 PrecFS <- mean(T1FS/(T1FS + F1FS))
 PrecInd; PrecMC1; PrecFS

 #' ### Sensitivity (Recall)
 SensInd <- mean(T1Ind)/lenDays1
 SensMC1 <- mean(T1MC1)/lenDays1
 SensFS <- mean(T1FS)/lenDays1
 SensInd; SensMC1; SensFS

 #' ### Specificity
 SpecInd <- 1 - mean(F1Ind)/(ndays - lenDays1)
 SpecMC1 <- 1 - mean(F1MC1)/(ndays - lenDays1)
 SpecFS <- 1 - mean(F1FS)/(ndays - lenDays1)
 SpecInd; SpecMC1; SpecFS

 #' ### F1-Score
 F1ScoreInd <- 2 * (PrecInd * SensInd) / (PrecInd + SensInd)
 F1ScoreMC1 <- 2 * (PrecMC1 * SensMC1) / (PrecMC1 + SensMC1)
 F1ScoreFS <- 2 * (PrecFS * SensFS) / (PrecFS + SensFS)
 F1ScoreInd; F1ScoreMC1; F1ScoreFS
 ...
 performance_table <- data.frame(
   ...
   Formula = formula,
   indc = round(c(AccInd, PrecInd, SensInd, SpecInd, F1ScoreInd) * 100, 2),
   mc1c = round(c(AccMC1, PrecMC1, SensMC1, SpecMC1, F1ScoreMC1) * 100, 2),
   fsc = round(c(AccFS, PrecFS, SensFS, SpecFS, F1ScoreFS) * 100, 2),
   check.names = FALSE
 )
 names(performance_table) <- c("Metric", "Formula", "Ind (\\%)", "MC2 (\\%)", "FS (\\%)")
