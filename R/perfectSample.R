...
perfectSample.MTD <- function(MTD, N = NULL) {
  ...
  if (!is.null(N)) {
    if ( !is.numeric(N) || length(N) != 1 || N %% 1 != 0 || N <= max(MTD$Lambda) ) {
      stop("N must be NULL or a positive integer greater than max(MTD$Lambda)=", max(MTD$Lambda),".")
    }
  } else {
  ...
  # Precompute cumulative distributions for efficiency
  ...
  repeat{
    ...
    } # Finished sampling all symbols in chain[Y].
