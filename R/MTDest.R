  ...
  repeat{
    ...
    if(indep){
      for (i in seq_len(lenA)) {
        end_p0[i] <- sum( (NPSja %>% dplyr::filter(a == A[i]))$NXP0xa ) / end_p0[i]
      }
    }
    ...
    for (j in seq_len(lenS)) {
      ...
      for (i in seq_len(lenA)) {
        aux_NPSja <- NPSja %>% dplyr::filter(NPSja[ ,j] == A[i]) #fix past x_j=A[i]
        aux_pj[i, ] <- sum( aux_NPSja %>% dplyr::select_at(paste0("NXP", S[j], "xa")) )
        for (k in seq_len(lenA)) {
          aux_pj[i, k] <- sum(aux_NPSja %>% dplyr::filter(a == A[k]) %>%
